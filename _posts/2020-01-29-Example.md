...
```
pm.test("Check CDE is true", function () {
    var jsonData = pm.response.json();
    pm.expect(jsonData.CdeConnected).to.eql(true);
});
```
Check values within an array:
```
pm.test("First group in list is APD Test 1", function () {
    var json = pm.response.json();
    pm.expect(json[0].Id).to.eql("tel:+919988776655;kn-corp-groups=5_80");
    pm.expect(json[0].Name).to.eql("APD Test 1");
    pm.expect(json[0].State).to.eql(3);
    1pm.expect(json[0].GroupType).to.eql(1);
});
```
